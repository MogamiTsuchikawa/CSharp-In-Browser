...
class CSharpBrowserCompiler {
    constructor(options = {}) {
        // Detect if we're running from node_modules
        const isNpmPackage = typeof window !== 'undefined' && 
            (window.location?.pathname.includes('node_modules') || 
             options.runtimePath?.includes('node_modules'));
        
        const defaultRuntimePath = isNpmPackage ? 
            'node_modules/csharp-browser-compiler/runtime/' : 
            (options.runtimePath || './');
            
        this.options = {
            monoConfig: options.monoConfig || `${defaultRuntimePath}mono-config.js`,
            runtimeJs: options.runtimeJs || 'runtime.js',
            dotnetJs: options.dotnetJs || `${defaultRuntimePath}dotnet.js`,
            dotnetWasm: options.dotnetWasm || `${defaultRuntimePath}dotnet.wasm`,
            assembliesPath: options.assembliesPath || `${defaultRuntimePath}managed/`,
            basePath: options.basePath || defaultRuntimePath, // Custom base path for asset loading
